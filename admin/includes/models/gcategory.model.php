...
class GcategoryModel extends BaseModel
{
    ...
    /* 补充分类字段 */
    public function addCateFields($gcategorys) {
        foreach ($gcategorys as $k => $v) {
            $sql = "select parent_id,root_image,cate_image from {$this->table} where cate_id = ".$v['id'];
            $arr_cate_info = $this->getRow($sql);
            if ($arr_cate_info['parent_id'] == 0) {
                $gcategorys[$k]['root_image'] = $arr_cate_info['root_image'];
            } else {
                $gcategorys[$k]['cate_image'] = $arr_cate_info['cate_image'];
            }
            if($gcategorys[$k]['children']) {
                $gcategorys[$k]['children'] = $this->addCateFields($v['children']);
            }
        }
        return $gcategorys;
    }
...
class GcategoryBModel extends GcategoryModel
{
    ...
    function drop($conditions, $fields = 'ad_image,root_image,cate_image')
    {
        ...
        if ($droped_rows)
        {
            ...
            foreach ($droped_data as $key => $value)
            {
                if ($value['ad_image'])
                {
                    @unlink(ROOT_PATH . '/' . $value['ad_image']);  //删除Logo文件
                }
                if ($value['root_image'])
                {
                    @unlink(ROOT_PATH . '/' . $value['root_image']);  //删除Logo文件
                }
                if ($value['cate_image'])
                {
    ...
    /* 获取同级分类的分类信息 by zj */
    function get_same_level_cate($id) {
        $res = array();
        if ($id > 0)
        {
            $sql = "select parent_id from {$this->table} WHERE cate_id = '$id' AND store_id = '{$this->_store_id}'";
            $parent_id = $this->getOne($sql);
            $sql = "SELECT cate_id, cate_name FROM {$this->table} WHERE parent_id = '$parent_id' AND cate_id <> '$id' AND store_id = '{$this->_store_id}'";
            $res = $this->getAll($sql);
        }
        return $res;
    }
    ...
    function get_ad_image($id) {
        ...
        if ($id > 0)
        {
            $sql = "select ad_image from {$this->table} WHERE cate_id = '$id' AND store_id = '{$this->_store_id}'";
            $res = $this->getOne($sql);
        }
        return $res;
    }
