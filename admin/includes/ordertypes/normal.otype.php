...
class NormalOrder extends BaseOrder
{
    ...
    function get_order_detail($order_id, $order_info)
    {
        ...
        {
            $arr_goods_info = $mod_goods->getRow('select goods_weight,rate from ecm_goods where goods_id = '.$goods['goods_id']);
            $data['goods_list'][$key]['weight'] = $arr_goods_info['goods_weight'] * $goods['quantity'];
            $data['goods_list'][$key]['rate'] = $arr_goods_info['rate'];
            $ratemoney += round(($goods['price'] * $arr_goods_info['rate']) * $goods['quantity']/100,2);
        }
        $data['ratemoney'] = ($ratemoney <= 50) ? 0 : $ratemoney;
        /* 配关信息 */
    ...
    function submit_order($data)
    {
        ...
        $order_model =& m('order');
		$base_info['card_id'] = $post['card_id'];
