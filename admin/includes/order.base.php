...
class BaseOrder extends Object
{
    ...
    function _get_my_address($user_id)
    {
        ...
        $arr_address = $address_model->find('user_id=' . $user_id);
        foreach ($arr_address as $key => $val) {
            $region_name = '';
            if ($val['province_id']) {
                $sql = "SELECT `region_name` FROM `ecm_region` WHERE region_id = " . $val['province_id'];
                $region_name .= $address_model->getOne($sql);
            }
            if ($val['city_id']) {
                $sql = "SELECT `region_name` FROM `ecm_region` WHERE region_id = " . $val['city_id'];
                $region_name .= ' '.$address_model->getOne($sql);
            }
            if ($val['area_id']) {
                $sql = "SELECT `region_name` FROM `ecm_region` WHERE region_id = " . $val['area_id'];
                $region_name .= ' '.$address_model->getOne($sql);
            }
            $arr_address[$key]['region_name'] = $region_name;
        }
        return $arr_address;
    }
    ...
    function _get_goods_list($order_id)
    {
        ...
        $arr_ordergoods = $ordergoods_model->find("order_id={$order_id}");
        return $arr_ordergoods;
    }
    ...
    function _get_order_extm($order_id)
    {
        ...
        $arr_orderextm = $orderextm_model->get($order_id);
        //todo 收货地址
        return $arr_orderextm;
    }
    ...
    function _handle_consignee_info($goods_info, $post)
    {
		...
		*/
		...
		$region_m = & m("region");
		$region = $region_m->get($consignee_info['area_id']);
		$region_p = $region_m->get($region['parent_id']);
		$region_pp = $region_m->get($region_p['parent_id']);
        if ($region_pp['region_name'] == '中国') {
            $region_name = '中国';
        } elseif (!in_array($region_p['parent_id'],array('2','25','27','32','33','34'))) {
            $region_name = $region_pp['region_name'].'省'.$region_p['region_name'].'市'.$region['region_name'];
        } else {
            $region_name = $region_pp['region_name'].'市'.$region['region_name'];
        }
        return array(
            'consignee'     =>  $consignee_info['consignee'],
            'region_id'     =>  $consignee_info['area_id'],
