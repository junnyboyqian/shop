...
class GcategoryApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            ...
            $image_list = array();
            /* 处理上传的图片 */
            $cate_adimg       =   $this->_upload_adimg($cate_id);
            $cate_adimg and $image_list['ad_image'] = $cate_adimg;
            /*if ($cate_adimg === false)
            {
                return;
            }*/
            $root_image       =   $this->_upload_rootimg($cate_id);
            $root_image and $image_list['root_image'] = $root_image;
            /*if ($root_image === false)
            {
                return;
            }*/
            $cate_image       =   $this->_upload_cateimg($cate_id);
            $cate_image and $image_list['cate_image'] = $cate_image;
            /*if ($cate_image === false)
            {
                return;
            }*/
            $image_list && $this->_gcategory_mod->edit($cate_id, $image_list); //将图片地址记下
     ...
     *    @return    string
     */
    function _upload_adimg($cate_id)
    {
        $file = $_FILES['cate_adimg'];
        ...
        $uploader->addFile($_FILES['cate_adimg']);//上传logo
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/cate', $cate_id))   //保存到指定目录，并以指定文件名$cate_id存储
        {
            return $file_path;
        }
        else
     ...
     *    @return    string
     */
    function _upload_rootimg($cate_id)
    {
        $file = $_FILES['root_image'];
        if ($file['error'] == UPLOAD_ERR_NO_FILE) // 没有文件被上传
        {
            return '';
        }
        ...
        $uploader = new Uploader();
        $uploader->allowed_type(IMAGE_FILE_TYPE); //限制文件类型
        $uploader->addFile($_FILES['root_image']);//上传logo
        if (!$uploader->file_info())
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=gcategory&amp;act=edit&amp;id=' . $cate_id);
            return false;
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/cate/root', $cate_id))   //保存到指定目录，并以指定文件名$cate_id存储
        {
            return $file_path;
        }
        else
     ...
     *    @return    string
     */
    function _upload_cateimg($cate_id)
    {
        $file = $_FILES['cate_image'];
        if ($file['error'] == UPLOAD_ERR_NO_FILE) // 没有文件被上传
        {
            return '';
        }
        ...
        $uploader = new Uploader();
        $uploader->allowed_type(IMAGE_FILE_TYPE); //限制文件类型
        $uploader->addFile($_FILES['cate_image']);//上传logo
        if (!$uploader->file_info())
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=gcategory&amp;act=edit&amp;id=' . $cate_id);
            return false;
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/cate/img', $cate_id))   //保存到指定目录，并以指定文件名$cate_id存储
        {
            return $file_path;
        }
        else
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            if ($gcategory['ad_image'])
            {
                $gcategory['ad_image']  =   dirname(site_url()) . "/" . $gcategory['ad_image'];
            }
            if ($gcategory['root_image'])
            {
                $gcategory['root_image']  =   dirname(site_url()) . "/" . $gcategory['root_image'];
            }
            if ($gcategory['cate_image'])
            {
                $gcategory['cate_image']  =   dirname(site_url()) . "/" . $gcategory['cate_image'];
            }
            $this->assign('gcategory', $gcategory);
            /* 导入jQuery的表单验证插件 */
            $this->import_resource(array(
        ...
        else
        {
            ...
            $cate_adimg   =   $this->_upload_adimg($id);
            $cate_adimg && $data['ad_image'] = $cate_adimg;
            $root_image   =   $this->_upload_rootimg($id);
            $root_image && $data['root_image'] = $root_image;
            $cate_image   =   $this->_upload_cateimg($id);
            $cate_image && $data['cate_image'] = $cate_image;
            if ($logo === false)
            {
                return;
            }
