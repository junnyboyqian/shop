...
class BrandApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            ...
            $data['brand_link']     = $_POST['brand_link'];
            $data['sort_order']     = $_POST['sort_order'];
            $data['recommended']    = $_POST['recommended'];
            $data['discount']    = $_POST['discount'];
            $data['discount_time']    = strtotime($_POST['discount_time']);
            $data['age']    = $_POST['age'];
            $data['tag'] = $_POST['tag'];
            $data['if_show'] = 1;
            ...
            $rec_img_url       =   $this->_upload_recommend($brand_id);
            if ($recommendimg === false)
            {
                return;
            }
            $rec_img_url && $this->_brand_mod->edit($brand_id, array('brand_recommend' => $rec_img_url)); //将推荐地址记下
    ...
    function edit()
    {
         ...
         if (!IS_POST)
        {
            ...
            {
                $brand['brand_recommend']  =   dirname(site_url()) . "/" . $brand['brand_recommend'];
            }
            if ($brand['discount_time'])
            {
                $brand['discount_time']  =  date('Y-m-d',$brand['discount_time']);
            }
            /* 显示新增表单 */
            $yes_or_no = array(
                1 => Lang::get('yes'),
        ...
        else
        {
            ...
            $rec_img_url              =   $this->_upload_recommend($brand_id);
            $rec_img_url && $data['brand_recommend'] = $rec_img_url;
            if ($rec_img_url === false)
            {
                return;
            }
             /* 检查名称是否已存在 */
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('brand_link','brand_name', 'recommended', 'sort_order', 'tag')))
       {
           $data[$column] = $value;
     ...
     *    @return    string
     */
    function _upload_recommend($brand_id)
    {
        $file = $_FILES['recommendimg'];
        if ($file['error'] == UPLOAD_ERR_NO_FILE) // 没有文件被上传
        {
            return '';
        }
        ...
        $uploader = new Uploader();
        $uploader->allowed_type(IMAGE_FILE_TYPE); //限制文件类型
        $uploader->addFile($_FILES['recommendimg']);
        if (!$uploader->file_info())
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=brand&amp;act=edit&amp;id=' . $brand_id);
            return false;
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/brand/rec', $brand_id))   //保存到指定目录，并以指定文件名$brand_id存储
        {
            return $file_path;
        }
        else
