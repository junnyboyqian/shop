...
class ecsTemplate
{
    ...
    {
        $this->_errorlevel = 'E_ERROR | E_WARNING | E_PARSE';
        $this->_nowtime    = time();
    }
    ...
    function get_val($val)
    {
        ...
        if (strrpos($val, '[') !== false)
        {
            ...
            //$val = preg_replace_callback($pattern, function($r) { return 与$r相关的值; }, $val);
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $source = preg_replace_callback('/{nocache}(.+?){\/nocache}/is', function($match){return "'{insert name=\"nocache\" ' . '" . $this->_echash . "' . base64_encode($match[1]) . '}'";}, $source);
        return $source;
    }
